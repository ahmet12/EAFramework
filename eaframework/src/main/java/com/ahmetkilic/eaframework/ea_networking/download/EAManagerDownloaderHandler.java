...
class EAManagerDownloaderHandler {
    ...
    void startDownload() {
        ...
        if (folderCreated && logListener != null)
            logListener.onLogRequired(LogType.INFORMATION, "Downloads folder is created");
    ...
    private String getSubFolderWithSeparator(boolean includeSeparatorBefore) {
        String path = "";
        ...
        } else if (includeSeparatorBefore)
            path = File.separator;
    ...
    private void queryStatus() {
        String error = contextWeakReference.get().getString(R.string.error_unknown);
        ...
        if (c == null) {
            ...
            if (isColumnContained(c) && c.getCount() > 0) {
                ...
                error = contextWeakReference.get().getString(R.string.error_cancelled);
                deliverResult(false, error);
            }
        }
    }
                ...
                while (true) {
                    ...
                    if (c != null && c.getCount() > 0) {
                        ...
                        try {
                            ...
                            listener.onDownloadProgress(dl_progress);
                        }catch (Exception e){
                            e.printStackTrace();
                        }
